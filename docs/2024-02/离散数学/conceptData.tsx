...
const conceptList: ConceptData[] = [
    ...
    },
    {
        ...
        \\end{cases}
        $$
        基本关联矩阵$M_f(G)$：从$M(G)$中删除任意一行所得矩阵称为G的基本关联矩阵。
        参考点：$M_f(G)$中被删行所对应的顶点`,
        front_log: ['有向图', '无向图', '环'],
        property_list: [
            '设G有$p$个连通分支，则$r(M(G))=r\\{M_f(G)\\}=n-p$，其中$M_f(G)$是从$M(G)$每个对角块删除任意一行得到的',
            '$M_f\`$是由$M_f(G)$中$n-1$列组成的方阵，$n-1$列对应边的导出子图是树当且仅当$|M_f\`|\\neq0$',
        ]
    },
    {
        name: "邻接矩阵",
        explain: `元素$a_{ij}$为$v_i$邻接到$v_j$的边的条数，
        记作$A(D)$简记$A$`,
    },
    {
        name: "可达矩阵",
        explain: `记为$P(G)$，$p_{ij}$为元素，代表$p_i$可达（或连通）$p_j$`,
        other_name: ['连通矩阵']
    },
    {
        name: "最优二叉树",
